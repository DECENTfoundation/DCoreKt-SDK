...
//  override val useMock: Boolean = false
  ...
  @Test fun `should get account history`() {
    ...
    val test = api.historyApi.listOperations(account)
  ...
  @Test fun `should get relative account history`() {
    ...
    val test = api.historyApi.listOperationsRelative(account, limit = 10)
        .subscribeOn(Schedulers.newThread())
        .test()
  ...
  @Test fun `should get account history balances`() {
    ...
    val test = api.historyApi.findAllOperations(account, startOffset = 2, limit = 3)
        .subscribeOn(Schedulers.newThread())
        .test()
  ...
  @Test fun `should get account balance history for op`() {
    ...
    val test = api.historyApi.getOperation(account, "1.7.7570".toChainObject())
        .subscribeOn(Schedulers.newThread())
        .test()
