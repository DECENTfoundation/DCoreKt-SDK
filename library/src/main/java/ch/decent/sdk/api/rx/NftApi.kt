...
class NftApi internal constructor(api: DCoreApi) : BaseApi(api) {
   ...
   * @return NFT data objects, or [ObjectNotFoundException] if none found
   */
  fun getAllData(ids: List<ChainObject>): Single<List<NftData<NftModel>>> = getAllDataRaw(ids).make()
   ...
   * @return NFT data object, or [ObjectNotFoundException] if none found
   */
  fun getData(id: ChainObject): Single<NftData<NftModel>> = getAllData(listOf(id)).map { it.single() }
   ...
   */
  @JvmOverloads
  fun getNftBalances(account: ChainObject, nftIds: List<ChainObject> = emptyList()): Single<List<NftData<NftModel>>> =
   ...
   * @return NFT data objects
   */
  fun listDataByNft(nftId: ChainObject): Single<List<NftData<NftModel>>> = listDataByNftRaw(nftId).make()
