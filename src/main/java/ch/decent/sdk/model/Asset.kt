...
data class Asset(
  ...
  fun fromBase(value: BigInteger) = value.toBigDecimal().divide(BigDecimal.TEN.pow(precision))
  fun toBase(value: BigDecimal) = value.multiply(BigDecimal.TEN.pow(precision)).toBigInteger()
  ...
  fun parse(value: String): AssetAmount = AssetAmount(toBase(BigDecimal(value)), id)
