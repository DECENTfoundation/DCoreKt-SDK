...
internal val socket = RxWebSocket(TrustAllCerts.wrap(client).build(), url, logger = Logger.getLogger("RxWebSocket"), gson = DCoreApi.gsonBuilder.create())
val api = DCoreApi(
//    restUrl = restUrl,
    restUrl = null,
    webSocketUrl = url,
    client = TrustAllCerts.wrap(client).build(),
    logger = Logger.getLogger("DCoreApi")
)
val account = ChainObject.parse("1.2.30")
val account2 = ChainObject.parse("1.2.31")
